...
use Libadalang.Rewriting_Implementation;

private package Libadalang.Unparsing_Implementation is
   ...
   is record
      case Kind is
         when From_Parsing =>
            Parsing_Node : Bare_Ada_Node;
         when From_Rewriting =>
            Rewriting_Node : Node_Rewriting_Handle;
      end case;
   end record;
   ...
   function Kind (Node : Abstract_Node) return Ada_Node_Kind_Type;
   ...
   function Rewritten_Node (Node : Abstract_Node) return Bare_Ada_Node;
   --  If Node is a parsing node, return it. If Node is a rewritten node,
   --  return the original node (i.e. of which Node is a rewritten version),
   --  or null if there is no original node.
   ...
   procedure Append
     (Buffer : in out Unparsing_Buffer; Kind : Token_Kind;
      Text   :        Text_Type) with
      Pre => Text'Length > 0;
   ...
   type Token_Unparser is record
      ...
      --  specific literal: for such token kinds, this must be the text to
      --  emit for that token.
   end record;
   ...
   type Node_Unparser_Map is array (Ada_Node_Kind_Type) of Node_Unparser;
   type Node_Unparser_Map_Access is access constant Node_Unparser_Map;

   Node_Unparsers : Node_Unparser_Map_Access;
   --  Unparsing table, describing how to unparse all parse nodes in Ada.
