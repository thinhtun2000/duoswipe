...
with Libadalang.Lexer_State_Machine; use Libadalang.Lexer_State_Machine;
...
package body Libadalang.Lexer_Implementation is
   ...
   procedure Decode_Buffer
     (Buffer, Charset :     String; Read_BOM : Boolean;
      Decoded_Buffer  : out Text_Access; Source_First : out Positive;
   ...
   --  Invalid_Input if Buffer contains invalid byte sequences according
   --  to Charset.

   procedure Extract_Tokens_From_Text_Buffer
     (Decoded_Buffer :        Text_Access; Source_First : Positive;
      Source_Last    :    Natural; Tab_Stop : Positive; With_Trivia : Boolean;
      TDH            : in out Token_Data_Handler;
      Diagnostics    : in out Diagnostics_Vectors.Vector);
   --  Helper for the Extract_Tokens procedure

   procedure Extract_Tokens_From_Bytes_Buffer
     (Buffer, Charset :        String; Read_BOM : Boolean; Tab_Stop : Positive;
      With_Trivia     :        Boolean; TDH : in out Token_Data_Handler;
      Diagnostics     : in out Diagnostics_Vectors.Vector);
   ...
   procedure Process_All_Tokens
     (Input       : Text_Access; Input_First : Positive; Input_Last : Natural;
      Tab_Stop    :        Positive; TDH : in out Token_Data_Handler;
      Diagnostics : in out Diagnostics_Vectors.Vector);

   function Force_Symbol
     (TDH : Token_Data_Handler; T : in out Stored_Token_Data)
      return Symbol_Type;
   ...
   is
      ...
      procedure Append_Token (Data : Stored_Token_Data) with
         Inline;
         --  Append a token to TDH and update Last_Token_Was_Trivia accordingly

      procedure Append_Trivia (Data : Stored_Token_Data) with
         Inline;
         --  If trivia are disabled, do nothing. Otherwise, append a trivia to
         --  TDH and update Last_Token_Was_Trivia and the token/trivia mapping
         --  in TDH accordingly.
      ...
      begin
         ...
         TDH.Trivias.Append ((Has_Next => False, T => Data));
   ...
   begin
      ...
      while Has_Next (State) loop
         ...
         if Token_Id /= Ada_Termination then
            declare
               ...
               if TDH.Symbols /= No_Symbol_Table then
                  ...
                  begin
                     if Symbol_Res.Success then
                        ...
                        Append
                          (Diagnostics, Sloc_Range, Symbol_Res.Error_Message);
                     end if;
                  end;
               end if;

            when Ada_Comment | Ada_Lexing_Failure | Ada_Prep_Line |
              Ada_Whitespace =>
               Append_Trivia
                 ((Kind         => From_Token_Kind (Token_Id),
                   Source_First => Source_First, Source_Last => Source_Last,
                   Symbol       => null, Sloc_Range => Sloc_Range));
         ...
         Append_Token
           ((Kind => From_Token_Kind (Token_Id), Source_First => Source_First,
             Source_Last => Source_Last, Symbol => Symbol,
      ...
      case Input.Kind is
         when File =>
            declare
               ...
               Buffer : String (1 .. Last (Region)) with
                  Import  => True,
                  Address => Buffer_Addr;
            begin
               Extract_Tokens_From_Bytes_Buffer
         ...
         when Bytes_Buffer =>
            declare
               Bytes : String (1 .. Input.Bytes_Count) with
                  Import,
                  Address => Input.Bytes;
            begin
               Extract_Tokens_From_Bytes_Buffer
   ...
   begin
      ...
      declare
         BOM_Kind_To_Charset : constant array
           (UTF8_All .. UTF32_BE) of String_Access :=
         ...
         Actual_Charset : constant String :=
           (if BOM in UTF8_All .. UTF32_BE then BOM_Kind_To_Charset (BOM).all
            else Charset);
      begin
      ...
      Iconv
        (State, Buffer, Input_Index, Output (Output_Index .. Output'Last),
         Output_Index, Status);
   ...
   function Get_Symbol
     (Token : Token_Or_Trivia_Index; TDH : Token_Data_Handler)
      return Symbol_Type
   is
      ...
      Token_Data : constant Token_Data_Reference :=
        (if Token.Trivia = No_Token_Index then
           Token_Data_Reference (TDH.Tokens.Get_Access (Natural (Token.Token)))
   ...
   function Force_Symbol
     (TDH : Token_Data_Handler; T : in out Stored_Token_Data) return Symbols
     .Symbol_Type
   is
   begin
